...
- раздел **Настройки → Подключение** — безопасный просмотр параметров
  IntraService и Telegram без раскрытия токенов и паролей;
- раздел **Настройки → Доступ** — просмотр списка разрешённых пользователей
  (ФИО + Telegram ID);
...
> В текущем выпуске вкладки подключения и доступа работают только на чтение.
> Единственное управляющее действие панели над базами бота — явная отмена ещё
> не созданной заявки из очереди.
