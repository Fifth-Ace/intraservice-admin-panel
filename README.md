...
с отменой, состояние сервисов, журнал и безопасный обзор настроек. Она
...
- новый компонентный frontend на **Svelte 5 + TypeScript + Vite**;
- адаптивные тёмная и светлая темы, мобильное меню и доступная клавиатурная навигация;
- **живые метрики** из SQLite-баз бота, состояние каналов и сервисов;
- read-only страницы заявок, шаблонов решений, AI-цепочки, журнала и настроек;
- поиск и фильтры по шаблонам и журналу без раскрытия персональных данных;
- безопасный просмотр секретов только как `задан / не задан`;
- старый интерфейс сохранён на `/legacy/` как мгновенный fallback;
- запись конфигурации и whitelist остаётся opt-in и доступна в legacy-интерфейсе.

> Запись конфигурации выключена по умолчанию. При явном включении изменения идут
> через предпросмотр, подтверждение, резервную копию и атомарное сохранение.
...
Безопасный перечень переменных окружения с переносимыми примерами находится в
[`.env.example`](.env.example). Панель не загружает `.env` автоматически:
передавайте значения через окружение процесса или systemd unit.
...
| `PANEL_ENABLE_V2` | `1` | `0` возвращает legacy-интерфейс на `/` |
| `PANEL_ENABLE_CONFIG_WRITE` | `0` | `1` включает изменение конфигурации и whitelist |
...
|---|---|---|
| Обзор | `#/` | Метрики, каналы, сервисы и последние события |
| Заявки | `#/tickets` | Очередь обработки |
| Шаблоны | `#/templates` | Категории, поиск и формулировки решений |
| Система | `#/system` | Сервисы, API latency и AI-цепочка |
| Журнал | `#/log` | Безопасный аудит событий с фильтрами |
| Настройки | `#/settings` | Read-only обзор подключения, доступа и секретов |
| Legacy | `/legacy/` | Предыдущий интерфейс и opt-in управление настройками |
...
- не возвращает и не журналирует токены/пароли; локальные backup-копии env при
  включённой записи защищены правами `0600` и находятся в `data/config-backups`;
